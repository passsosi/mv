...
@section('content')
    ...
    @endphp
            <div class="col-lg-8 col-md-8 mx-auto mt-2">
              <form action="{{ route('category4Update', ['id' => $data->id]) }}" method="post" enctype="multipart/form-data">
                @csrf
                @error('main')
                ...
                <div>
                    @foreach ($SecondCategory as $elICat)
                    <a class="w-50 dropdown-toggle hidden-arrow btn btn-primary " href="#" id="navbarDropdownMenuLink" role="button" data-mdb-toggle="dropdown" aria-expanded="false">Категория<input type="text" id="id_category" name="id_category" class="form-control form-control-dropdown mt-3" readonly placeholder="id категории" value="{{ $elICat->id }}" /></a>
                    @endforeach
