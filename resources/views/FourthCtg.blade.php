...
@section('content')
  ...
  <div class="album bg-body-tertiary">
    ...
    <div class="container py-5">
      <h1 class="text-center mt-3">{{$category[0]->name}} – {{$category[0]->latName}}</h1>
      <div class="row row-cols-1 row-cols-sm-1 row-cols-md-2 g-3 mt-2">
              ...
              <h2>{{$el->name}}<br/>{{$el->latName}}</h2>
                <div class="d-flex justify-content-between align-items-center mt-3">
                  ...
                  <div class="order {{Auth::check() ? '' : 'd-none' }}">
                    <form action="{{ route('4Order') }}" method="post">
                      @csrf
        ...
        <script>
          ...
          {
            window.location.href = "{{ route('list4', ':categoryId') }}".replace(':categoryId', categoryId);
          }
          function update(categoryId) 
          {
            window.location.href = "{{ route('category4UpdateView', ':categoryId') }}".replace(':categoryId', categoryId);
          }
          function categoryDel(categoryId) 
          {
            window.location.href = "{{ route('category4Delete', ':categoryId') }}".replace(':categoryId', categoryId);
          }
        </script>
