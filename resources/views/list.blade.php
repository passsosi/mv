                        ...
                        <div class="card shadow-sm">
                            ...
                            @foreach ($images as $img)
                                @if (!$firstImageFound && $img->id_item === $el->id)
                                    ...
                                    @endphp
                                    <a href="#" onclick="redirect({{ $el->id }})"><img
                            ...
                            <div class="card-body">
                                ...
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    <div>
                                        <button type="button"
                                            class="btn btn-md btn-block btn-outline-secondary mt-2 {{ Auth::check() ? '' : 'd-none' }}"
                                            onclick="update({{ $el->id }})"><i class="fa fa-pencil"
                                                aria-hidden="true"></i></button>
                                        <button type="button"
                                            class="btn btn-md btn-block btn-outline-secondary mt-2 {{ Auth::check() ? '' : 'd-none' }}"
                                            onclick="itemDel({{ $el->id }})"><i class="fa fa-trash"
                                                aria-hidden="true"></i></button>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="">
                    <div class="card {{ Auth::check() ? '' : 'd-none' }}">
                        <button type="button" class="btn btn-md btn-block btn-outline-secondary"
                            onclick="add({{ $category[0]->id }})">Добавить</button>
